...
[РЕФЕРЕНС] Источник правды для аналитики перенесён в BigQuery SQL:
    sql/02_products_timeseries.sql (точный перенос этой логики).
Модуль оставлен как проверенная референсная реализация и для парити-чека
(сверка чисел BQ-вью с этим расчётом). В ежедневном пайплайне больше не
вызывается. Методологию НЕ менять без согласования.
